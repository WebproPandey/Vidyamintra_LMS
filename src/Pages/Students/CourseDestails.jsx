    ...
    <>
      <div className="flex md:flex-row flex-col-reverse gap-10 relative  items-start  justify-between  md:px-36 px-8 md:pt-30 pt-20 text-left">
        ...
        <div className="max-w-xl z-10 text-gray-500 ">
          ...
          <p
            className="md:text-base   pt-4 text-sm line-clamp-3 "
            ...
            <div className='pt-5'>
              {
                courseData.courseContent.map((chapter , index) => (
                  <div key={index} className='border border-gray-300  bg-white mb-2 rounded'>
                    <div onClick={() => toggelSection(index)} className='flex items-center  justify-between  px-4 py-3 cursor-pointer  select-none'>
                      <div className='flex items-center  gap-2'>
                        <img src={assets.down_arrow_icon}  className={`transform transition-transform ${opneSection[index] ? 'rotate-180' : '' }`} alt="" />
                        <p className='font-medium  md:text-base  text-sm '>{chapter.chapterTitle}</p>
                      </div>
                      <p>{chapter.chapterContent.length} lecture - {calculateChapterTime(chapter)} </p>
                    </div>
                    <div  className={`overflow-hidden  transition-all  duration-300 ${opneSection[index]? 'max-h-96' : ' max-h-0' }` }>
        ...
        <div className='max-w-[424px] shadow-[0px_4px_15px_2px_rgba(0,0,0,0.05)]  z-10 rounded-t md:rounded-none overflow-hidden  bg-white min-w-[324px] sm:min-w-[420px]'>
        {
                playerData ?  
